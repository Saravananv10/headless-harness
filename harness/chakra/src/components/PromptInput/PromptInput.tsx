...
import { permissionModeShortTitle } from '../../utils/permissions/PermissionMode.js';
...
import {
  PROMPT_INPUT_LEFT_ACCENT_BG,
  PROMPT_INPUT_SURFACE_BG,
} from './promptChromeColors.js';
...
/**
 * Prompt chrome — colors and layout knobs live here (top of `PromptInput.tsx`).
 * - `PROMPT_INPUT_LEFT_BAR_WIDTH`: strip thickness in character columns (e.g. 2 or 3).
 */
/** Filled vertical strip on the left (character columns wide). */
const PROMPT_INPUT_LEFT_BAR_WIDTH = 1;
/** Space between the strip and the input column. */
const PROMPT_INPUT_INNER_PADDING_X = 1;
/** Mode ribbon label only — permission short title in prompt mode (`Text color`). */
const PROMPT_INPUT_RIBBON_LABEL_COLOR = '#C8B6FF';
const PROMPT_INPUT_PADDING_Y = 1;

function promptRibbonLabel(mode: PromptInputMode, permissionMode: PermissionMode): string {
  switch (mode) {
    case 'bash':
      return 'Shell';
    case 'orphaned-permission':
      return 'Permission';
    case 'task-notification':
      return 'Task';
    default:
      return permissionModeShortTitle(permissionMode);
  }
}

function promptProviderLabel(): string {
...
function promptModelSummaryLine(resolvedModelId: string): string {
  const friendly = getMarketingNameForModel(resolvedModelId) ?? resolvedModelId;
  return `${friendly} ${promptProviderLabel()}`;
}
...
function PromptInput({
  ...
  useEffect(() => {
    if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
      addNotification({
        ...
        text: 'This prompt will launch an ultraplan session in Chakra on the web',
        priority: 'immediate',
        timeoutMs: 5000
      });
    } else {
  ...
  const standaloneSessionColor = useAppState(s => s.standaloneAgentContext?.color);
  const resolvedSessionThemeColor = standaloneSessionColor && AGENT_COLORS.includes(standaloneSessionColor as AgentColorName)
    ? AGENT_COLOR_TO_THEME_COLOR[standaloneSessionColor as AgentColorName]
    : undefined;
  const leftAccentBg: keyof Theme | typeof PROMPT_INPUT_LEFT_ACCENT_BG = resolvedSessionThemeColor ?? PROMPT_INPUT_LEFT_ACCENT_BG;
  const ribbonLabelColor: keyof Theme | typeof PROMPT_INPUT_RIBBON_LABEL_COLOR = resolvedSessionThemeColor ?? PROMPT_INPUT_RIBBON_LABEL_COLOR;
  ...
  const textInputColumns =
    columns -
    PROMPT_INPUT_LEFT_BAR_WIDTH -
    PROMPT_INPUT_INNER_PADDING_X -
    3 -
  ...
  if (isExternalEditorActive) {
    return <Box flexDirection="row" alignItems="stretch" width="100%" backgroundColor={PROMPT_INPUT_SURFACE_BG}>
      <Box width={PROMPT_INPUT_LEFT_BAR_WIDTH} minWidth={PROMPT_INPUT_LEFT_BAR_WIDTH} flexShrink={0} backgroundColor={leftAccentBg} alignSelf="stretch" />
      <Box flexGrow={1} paddingLeft={PROMPT_INPUT_INNER_PADDING_X} paddingRight={1} paddingTop={PROMPT_INPUT_PADDING_Y} paddingBottom={PROMPT_INPUT_PADDING_Y} justifyContent="center">
  ...
  return <Box flexDirection="column" marginTop={briefOwnsGap ? 0 : 1}>
    ...
    {swarmBanner?.text ? <Text color={swarmBanner.bgColor}>
      <Text backgroundColor={swarmBanner.bgColor} color="inverseText">
        {' '}
        {swarmBanner.text}{' '}
      </Text>
    </Text> : null}
    <Box flexDirection="row" width="100%" backgroundColor={leftAccentBg} paddingLeft={PROMPT_INPUT_LEFT_BAR_WIDTH}>
      <Box flexDirection="column" flexGrow={1} flexShrink={1} backgroundColor={PROMPT_INPUT_SURFACE_BG} paddingLeft={PROMPT_INPUT_INNER_PADDING_X} paddingRight={1} paddingTop={PROMPT_INPUT_PADDING_Y} paddingBottom={PROMPT_INPUT_PADDING_Y}>
        <Box flexDirection="row" alignItems="flex-start">
          <PromptInputModeIndicator mode={mode} isLoading={isLoading} viewingAgentName={viewingAgentName} viewingAgentColor={viewingAgentColor} standaloneSessionColor={standaloneSessionColor} />
          <Box flexGrow={1} flexShrink={1} onClick={handleInputClick}>
            {textInputElement}
          </Box>
        </Box>
        <Box marginTop={1}>
          <Text color={ribbonLabelColor}>{promptRibbonLabel(mode, effectiveToolPermissionContext.mode)}</Text>
          <Text dimColor> {promptModelSummaryLine(mainLoopModel)}</Text>
        </Box>
      </Box>
    </Box>
    {/* {swarmBanner?.text ? <Text color={swarmBanner.bgColor}></Text> : null} */}
