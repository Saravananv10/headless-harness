...
import { PROMPT_INPUT_SURFACE_BG } from './promptChromeColors.js'
...
const SuggestionItemRow = memo(function SuggestionItemRow({
  ...
  const rowBackgroundColor: keyof Theme | typeof PROMPT_INPUT_SURFACE_BG | undefined =
    isSelected ? 'suggestion' : overlay ? PROMPT_INPUT_SURFACE_BG : undefined
  const textColor: keyof Theme | undefined = isSelected ? 'inverseText' : undefined
  const useOpaqueIdleFill = rowBackgroundColor === undefined
  ...
  if (isUnifiedSuggestion(item.id)) {
    ...
    return (
      <Box width="100%" opaque={useOpaqueIdleFill} backgroundColor={rowBackgroundColor}>
