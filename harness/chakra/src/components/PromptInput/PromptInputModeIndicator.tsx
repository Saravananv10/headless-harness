...
type Props = {
  ...
  standaloneSessionColor?: AgentColorName;
};
...
export function PromptInputModeIndicator(t0) {
  ...
  const standaloneSessionThemeColor = standaloneSessionColor ? AGENT_COLOR_TO_THEME_COLOR[standaloneSessionColor] : undefined;
  let t2;
  if ($[1] !== isLoading || $[2] !== mode || $[3] !== viewedTeammateThemeColor || $[4] !== viewingAgentName || $[5] !== standaloneSessionThemeColor) {
    t2 = <Box alignItems="flex-start" alignSelf="flex-start" flexWrap="nowrap" justifyContent="flex-start">{viewingAgentName ? <PromptChar isLoading={isLoading} themeColor={viewedTeammateThemeColor} /> : mode === "bash" ? <Text color="bashBorder" dimColor={isLoading}>! </Text> : <PromptChar isLoading={isLoading} themeColor={standaloneSessionThemeColor ?? (isAgentSwarmsEnabled() ? teammateColor : undefined)} />}</Box>;
    $[1] = isLoading;
    $[2] = mode;
    $[3] = viewedTeammateThemeColor;
    $[4] = viewingAgentName;
    $[5] = standaloneSessionThemeColor;
    $[6] = t2;
  } else {
    t2 = $[6];
  }
