...
export type SpinnerAnimationRowProps = {
  ...
  /** Forward animation cycle for the glyph column */
...
export function SpinnerAnimationRow({
  ...
  const glyphColumnWidth = useMemo(
    () => Math.max(2, ...spinnerFrames.map(f => stringWidth(f))),
    [spinnerFrames],
  );
  ...
  const messageWidth = glimmerMessageWidth + glyphColumnWidth + 1;
  const sep = SEP_WIDTH;
  ...
  return <FullWidthRow>
      <Box ref={viewportRef} flexDirection="row" flexWrap="wrap" marginTop={1}>
        <SpinnerGlyph frame={frame} messageColor={messageColor} stalledIntensity={overrideColor ? 0 : stalledIntensity} reducedMotion={reducedMotion} time={time} frames={spinnerFrames} />
