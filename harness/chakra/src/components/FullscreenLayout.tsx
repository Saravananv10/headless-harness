...
import { PROMPT_INPUT_SURFACE_BG } from './PromptInput/promptChromeColors.js';
 ...
 * until Chakra's text response lands.
 */
...
//
// left=1 + paddingLeft=1 keeps column 0 clear for the prompt's purple accent
// strip; backgroundColor matches the prompt surface so any spill isn't
// terminal-default (white). `opaque` is avoided — it fills with unstyled
// spaces and clobbered that strip after menus closed.
function SuggestionsOverlay() {
  ...
  if ($[0] !== data.maxColumnWidth || $[1] !== data.selectedSuggestion || $[2] !== data.suggestions) {
    t0 = <Box position="absolute" bottom="100%" left={1} right={0} paddingLeft={1} paddingRight={2} paddingTop={1} flexDirection="column" backgroundColor={PROMPT_INPUT_SURFACE_BG}><PromptInputFooterSuggestions suggestions={data.suggestions} selectedSuggestion={data.selectedSuggestion} maxColumnWidth={data.maxColumnWidth} overlay={true} /></Box>;
...
function DialogOverlay() {
  ...
  let t0;
  if ($[0] !== node) {
    t0 = <Box position="absolute" bottom="100%" left={1} right={0} backgroundColor={PROMPT_INPUT_SURFACE_BG}>{node}</Box>;
    $[0] = node;
    $[1] = t0;
  } else {
    t0 = $[1];
