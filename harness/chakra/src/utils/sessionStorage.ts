...
/**
 * Reads the agent color from the most recent session's transcript file.
 * Returns undefined if no color entry is found.
 */
export async function getLastSessionAgentColor(projectDir: string): Promise<string | undefined> {
  const sessionFilesMap = await getSessionFilesWithMtime(projectDir)
  ...
  // Sort by mtime descending to get the most recent session
  const entries = [...sessionFilesMap.entries()].sort((a, b) => b[1].mtime - a[1].mtime)
  const [mostRecentSessionId] = entries[0]!

  const sessionFile = join(projectDir, `${mostRecentSessionId}.jsonl`)

  try {
    const fileContent = await readFile(sessionFile, 'utf-8')

    // Find the last agent-color entry
    const lines = fileContent.split('\n')
    for (let i = lines.length - 1; i >= 0; i--) {
      const line = lines[i]
      if (line && line.includes('"type":"agent-color"')) {
        const entry = jsonParse(line) as { type: string; agentColor: string; sessionId: string }
        if (entry.type === 'agent-color' && entry.agentColor !== 'default') {
          return entry.agentColor
        }
      }
    }
  } catch {
    // Best-effort: ignore errors reading the file
  }
