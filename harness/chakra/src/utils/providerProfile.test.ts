...
const missingCodexAuthPath = join(tmpdir(), 'chakra-missing-codex-auth.json')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'chakra-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'chakra-profile-file-'))
...
test('auto profile falls back to soket when no viable ollama model exists', () => {
  assert.equal(selectAutoProfile(null), 'soket')
