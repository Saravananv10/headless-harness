 ...
 *
 * Usage:
 *   chakra --provider soket --model gpt-4o
 *   chakra --provider openai --model gpt-4o
 *   chakra --provider gemini --model gemini-2.0-flash
 *   chakra --provider ollama --model llama3.2
 *   chakra --provider anthropic   (use Anthropic API)
 */
...
export function applyProviderFlag(
  ...
  switch (provider as ProviderFlagName) {
    case 'soket':
      process.env.CLAUDE_CODE_USE_SOKET = '1'
      process.env.OPENAI_BASE_URL ??= DEFAULT_SOKET_BASE_URL
      if (model) process.env.OPENAI_MODEL ??= model
      break

    case 'anthropic':
      process.env.CLAUDE_CODE_USE_FIRST_PARTY = '1'
      break
