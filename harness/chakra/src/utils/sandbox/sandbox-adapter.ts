...
/**
 * Resolve Chakra-specific path patterns for sandbox-runtime.
 *
 * Chakra uses special path prefixes in permission rules:
...
/**
 * Convert Chakra settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // This blocks settings in the original working directory (where Chakra started)
  ...
  // core.fsmonitor) escapes the sandbox when Chakra's unsandboxed git runs.
  //
 ...
 * Chakra's unsandboxed git calls can see them. See the SECURITY block above
