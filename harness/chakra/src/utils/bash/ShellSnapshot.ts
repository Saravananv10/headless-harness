...
/**
 * Generates Chakra specific snapshot content
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Chakra's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Chakra content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Chakra defaults only`,
        )
      }
