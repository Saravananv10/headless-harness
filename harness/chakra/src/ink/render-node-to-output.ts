...
function renderNodeToOutput(
  ...
  if (yogaNode) {
    ...
    const layoutHeight = yogaNode.getComputedHeight()
    let height = layoutHeight
    // When true, children must be clipped to the shortened paint rect — yoga
    // still laid them out for the full layoutHeight.
    let clipAbsoluteToViewport = false
    ...
    // (best matches in an autocomplete). Clamping y to 0 must also shorten the
    // painted height by the same amount so the bottom edge stays fixed; keeping
    // the full yoga height made the box grow downward and opaque fills
    // overwrote the prompt chrome (e.g. the left accent strip after closing `/`
    // menus). Scroll containers keep the legacy y-only clamp — their viewport
    // math uses layout height in ways that must stay aligned with yoga.
    if (y < 0 && node.style.position === 'absolute') {
      if (node.nodeName === 'ink-box') {
        ...
        if (isScrollY) {
          ...
          const newH = Math.max(0, Math.floor(height + y))
          clipAbsoluteToViewport = newH < height
          height = newH
          y = 0
        }
      } else {
