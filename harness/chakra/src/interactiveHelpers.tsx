...
import { getClaudeConfigHomeDir, isEnvTruthy, isRunningOnHomespace } from './utils/envUtils.js';
...
function isSoketMode(): boolean {
  return isEnvTruthy(process.env.CLAUDE_CODE_USE_SOKET) || (!isEnvTruthy(process.env.CLAUDE_CODE_USE_FIRST_PARTY) && !isEnvTruthy(process.env.CLAUDE_CODE_USE_GEMINI) && !isEnvTruthy(process.env.CLAUDE_CODE_USE_GITHUB) && !isEnvTruthy(process.env.CLAUDE_CODE_USE_OPENAI) && !isEnvTruthy(process.env.CLAUDE_CODE_USE_BEDROCK) && !isEnvTruthy(process.env.CLAUDE_CODE_USE_VERTEX) && !isEnvTruthy(process.env.CLAUDE_CODE_USE_FOUNDRY));
}

async function ensureSoketApiKeyViaSetupDialog(root: Root): Promise<void> {
  if (!isSoketMode()) return;
  const envKey = process.env.SOKET_API_KEY?.trim();
  if (envKey) {
    process.env.SOKET_API_KEY = envKey;
    return;
  }
  const {
    SoketApiKeyDialog
  } = await import('./components/SoketApiKeyDialog.js');
  const apiKey = await showSetupDialog<string>(root, done => <SoketApiKeyDialog onDone={done} />, {
    onChangeAppState
  });
  const {
    mkdir,
  ...
  await writeFile(join(home, 'soket_api_key'), `${key}\n`, {
    encoding: 'utf-8',
    mode: 0o600
  });
  process.env.SOKET_API_KEY = key;
}
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  await ensureSoketApiKeyViaSetupDialog(root);
  ...
  // processes but ignored by Chakra itself (see auth.ts).
