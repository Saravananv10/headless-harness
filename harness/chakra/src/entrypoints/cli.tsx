...
// Chakra: disable experimental API betas by default.
 ...
 */
async function hydrateSoketApiKeyFromDisk(): Promise<void> {
  ...
  const keyFilePath = join(getClaudeConfigHomeDir(), 'soket_api_key')

  try {
    const savedKey = (await readFile(keyFilePath, 'utf-8')).trim()
    if (savedKey) {
      process.env.SOKET_API_KEY = savedKey
    }
  } catch {
    // Missing/unreadable file: setup UI will handle first-run key capture.
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Chakra)`);
    return;
  }
  ...
  await hydrateSoketApiKeyFromDisk()
