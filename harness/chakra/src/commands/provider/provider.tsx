...
const DEFAULT_SOKET_BASE_URL = 'https://api.model.soket.ai/v1'
const DEFAULT_SOKET_MODEL = 'vidura-v1'
...
export function buildCurrentProviderSummary(options?: {
  ...
  if (isEnvTruthy(processEnv.CLAUDE_CODE_USE_SOKET)) {
    return {
      providerLabel: 'Soket',
      modelLabel: getSafeDisplayValue(
        processEnv.OPENAI_MODEL ?? DEFAULT_SOKET_MODEL,
        processEnv,
      ),
      endpointLabel: getSafeDisplayValue(
        processEnv.OPENAI_BASE_URL ?? DEFAULT_SOKET_BASE_URL,
        processEnv,
      ),
      savedProfileLabel,
    }
  ...
  switch (profile) {
    ...
    case 'soket':
      return {
        providerLabel: 'Soket',
        modelLabel: getSafeDisplayValue(
          env.OPENAI_MODEL ?? DEFAULT_SOKET_MODEL,
          process.env,
          env,
        ),
        endpointLabel: getSafeDisplayValue(
          env.OPENAI_BASE_URL ?? DEFAULT_SOKET_BASE_URL,
          process.env,
          env,
        ),
      }
...
export function buildProfileSaveMessage(
  ...
  lines.push('Restart Chakra to use it.')
...
function buildUsageText(): string {
  ...
  return [
    ...
    '',
    'Choose Auto, Ollama, OpenAI-compatible, Gemini, or Codex, then save a profile for the next Chakra restart.',
  ].join('\n')
}
...
function ProviderChooser({
  ...
  const options: OptionWithDescription<ProviderChoice>[] = [
    ...
    },
    {
      label: 'Soket',
      value: 'soket',
      description: 'Use Soket AI cloud models (Vidura, Eka, etc.)',
    },
    {
      label: 'Ollama',
      value: 'ollama',
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .chakra-profile.json and return to normal startup',
    })
  }
  ...
  return (
    ...
    >
      <Box flexDirection="column" gap={1}>
        <Text>
          Save a provider profile for the next Chakra restart without
...
function SoketModelStep({
  ...
  const options: OptionWithDescription<string>[] = [
    ...
    },
    {
      label: 'EKA',
      value: 'eka',
      description: '120B Math, Code and Reasoning Model',
    },
  ]

  return (
    <Dialog title="Choose a Soket model" onCancel={onBack}>
      <Box flexDirection="column" gap={1}>
        <Text>
          Pick a Soket AI model to use. No API key required for Soket cloud models.
        </Text>
        <Select
          options={options}
          defaultValue="Vidura-v1"
          defaultFocusValue="Vidura-v1"
          inlineDescriptions
          visibleOptionCount={options.length}
          onChange={value => {
            onSave(
              'soket',
              buildSoketProfileEnv({ model: value }),
            )
          }}
          onCancel={onBack}
        />
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart Chakra to go back to normal startup.`, {
                display: 'system',
              })
            } else {
