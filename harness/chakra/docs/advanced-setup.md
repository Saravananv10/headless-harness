...
```bash
git clone https://node.gitlawb.com/z6MkqDnb7Siv3Cwj7pGJq4T5EsUisECqR8KpnDLwcaZq5TPr/chakra.git
cd chakra
...
If you already use the Codex CLI, Chakra reads `~/.codex/auth.json` automatically. You can also point it elsewhere with `CODEX_AUTH_JSON_PATH` or override the token directly with `CODEX_API_KEY`.
...
# launch using persisted profile (.chakra-profile.json)
bun run dev:profile
