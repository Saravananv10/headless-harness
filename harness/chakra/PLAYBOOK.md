# Chakra Local Agent Playbook

This playbook is a practical guide to run Chakra with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Chakra starts directly.
...
Or let Chakra recommend the best local model for your goal:
...
```powershell
Get-Content .\.chakra-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\chakra\chakra
bun run dev:profile
```
...
- Keep `.chakra-profile.json` local (already gitignored).
