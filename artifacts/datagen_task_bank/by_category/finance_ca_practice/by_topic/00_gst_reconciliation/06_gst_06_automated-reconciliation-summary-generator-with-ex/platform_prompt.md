# AutomatedReconciliationSummary — Automated reconciliation summary generator with Excel export
...
## 1. Product identity
Build **AutomatedReconciliationSummary** for category `gst_reconciliation`. Seed intent (honor this product, do not genericize away):

> Create a Python script using pandas and openpyxl that reads ERP and GSTR CSV files, performs full reconciliation, and produces a formatted Excel workbook with multiple sheets: Summary, Matched Invoices, Mismatches by Type, Supplier-wise Breakdown, and ITC Impact Analysis. Include conditional formatting for highlighting discrepancies.

Artifact type: `spreadsheet_workbook`. Novelty hook: domain-specific twist. Delivery: `one_command_dev_server`.
