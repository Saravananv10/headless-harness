# PeriodWiseReconciliation — Period-wise reconciliation with trend analysis dashboard
...
## 1. Product identity
Build **PeriodWiseReconciliation** for category `gst_reconciliation`. Seed intent (honor this product, do not genericize away):

> Build a Python web app (Streamlit or Flask) that performs period-wise GST reconciliation across multiple months, shows trend charts for match rates, mismatch categories, ITC amounts, and supplier reliability scores over time. Include period comparison and drill-down views.

Artifact type: `web_fullstack`. Novelty hook: domain-specific twist. Delivery: `one_command_dev_server`.
