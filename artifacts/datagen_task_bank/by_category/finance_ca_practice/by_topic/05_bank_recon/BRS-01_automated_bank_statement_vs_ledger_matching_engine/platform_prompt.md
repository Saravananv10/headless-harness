# AutomatedBankStatementvsLedger — Automated Bank Statement vs Ledger Matching Engine
...
## 1. Product identity
Build **AutomatedBankStatementvsLedger** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Build a Python tool that matches bank statement transactions against general ledger entries by date, reference number, and amount, producing a Bank Reconciliation Statement (BRS).
