# SplitPurchaseOrderThresholdEva — Split Purchase Order Threshold Evasion Detector
...
## 1. Product identity
Build **SplitPurchaseOrderThresholdEva** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Build a Python script that analyzes purchasing logs for multiple purchase orders issued to the same vendor on the same day just below approval threshold limits.
