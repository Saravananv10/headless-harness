# ShellCompanyVendorRiskScorer — Shell Company Vendor Risk Scorer
...
## 1. Product identity
Build **ShellCompanyVendorRiskScorer** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Create a Python risk scoring tool that evaluates vendor master profiles against shell company indicators (recent registration, high payment volume, missing tax registration).
