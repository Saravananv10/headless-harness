# ManualvsSystemAutomatedJournal — Manual vs System-Automated Journal Entry Risk Profiler
...
## 1. Product identity
Build **ManualvsSystemAutomatedJournal** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Build a Python tool that segregates automated sub-ledger postings from manual journal entries, computes manual entry ratio per account, and flags high-risk manual accounts.
