# SuspiciousNarrationNLPPatternM — Suspicious Narration NLP Pattern Matcher
...
## 1. Product identity
Build **SuspiciousNarrationNLPPatternM** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Create a Python NLP text processing tool that scans journal entry narration strings for suspicious keywords ('temp', 'plug', 'adjust', 'per director', 'error fix'), outputting an exception list.
