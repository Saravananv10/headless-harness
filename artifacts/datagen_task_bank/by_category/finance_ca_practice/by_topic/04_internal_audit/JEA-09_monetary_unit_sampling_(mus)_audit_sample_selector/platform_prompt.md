# MonetaryUnitSampling(MUS)Audit — Monetary Unit Sampling (MUS) Audit Sample Selector
...
## 1. Product identity
Build **MonetaryUnitSampling(MUS)Audit** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Build a Python statistical sampling engine that performs Monetary Unit Sampling on general ledger populations, selecting high-value and random items for substantive testing.
