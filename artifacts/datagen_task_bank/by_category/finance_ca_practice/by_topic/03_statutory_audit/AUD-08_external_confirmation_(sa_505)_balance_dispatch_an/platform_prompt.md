# ExternalConfirmation(SA505)Bal — External Confirmation (SA 505) Balance Dispatch & Tracker
...
## 1. Product identity
Build **ExternalConfirmation(SA505)Bal** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Create a Python tool that selects debtor/creditor samples based on balance size and turnover, generates balance confirmation request letters, and tracks response status.
