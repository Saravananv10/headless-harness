# FixedAssetsRegister(FAR)vsBook — Fixed Assets Register (FAR) vs Books Reconciliation & Depreciation Checker
...
## 1. Product identity
Build **FixedAssetsRegister(FAR)vsBook** for category `finance_ca_practice`. Seed intent (honor this product, do not genericize away):

> Create a Python tool that matches Fixed Asset Register entries against Trial Balance asset accounts, recalculates Companies Act Schedule II depreciation, and flags discrepancies.
